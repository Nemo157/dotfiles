...
Use the Slack MCP tools to find conversations you participated in since `$SLACK_START_DATE`. Include **all channel types**: public channels, private channels, group DMs, and 1:1 DMs (including self-DMs).
...
Use `slack_search_public_and_private` with `channel_types` set to `public_channel,private_channel,mpim,im`:
...
**Step 4b — Handle self-DMs:**

Messages sent to yourself (your own DM channel) are self-reminders. These should **always** be filed, never skipped as low-signal. In particular, messages starting with "todo" (case-insensitive) are explicit self-reminder tasks — create a task file in `tasks/` for each one in addition to the journal entry. For non-todo self-DMs, still create journal entries as they represent notes-to-self worth preserving.
...
- Skip low-signal messages (simple reactions, one-word acknowledgements, bot notifications) — but **never skip self-DMs** (see step 4b)
...
For 1:1 DMs, use the person's name instead of a channel:

```markdown
### HH:MM (auto-generated by brain-sync)

Slack DM with [[people/@handle|Name]]: summary of the conversation.

#auto-sync
```

For self-DM reminders (messages starting with "todo"):

```markdown
### HH:MM (auto-generated by brain-sync)

Self-reminder: <todo content>. Filed as [[tasks/SLUG|task title]].
