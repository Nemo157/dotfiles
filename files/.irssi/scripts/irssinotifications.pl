...
%IRSSI   = (
    ...
    contact     => "ghostunderscore\@gmail.com",
    name        => "IrssiNotifications",
    description => "Send notifications about irssi highlights to os x notifier",
    ...
    if ($pid > 0) {
        ...
        eval {
            ...
            if ($type eq 'notification') {
                ...
                $notify_args = "-message 'Command: $command'";
            }

            my $result =  `$notifier_cmd $notify_args`;
            if ($? != 0) {
